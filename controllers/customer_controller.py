        ...
        if customers_list:
            ...
            if not customer_choice.lower() == "q":
                ...
                if customer_obj:
                    ...
                    permission = self.customer_model.check_permission_customer(
                        employee_id, customer_obj
                    )
                    if permission:
                        # .... if permit : display customer modification menu
                        customer_to_update_obj = self.customer_view.update_customer(
                            customer_obj
                        )
                ...
                if not customer_choice.lower() == "q":
                    ...
                    if customer_obj:
                        while choice.lower() != "o" and choice.lower() != "n":
                            ...
                            if choice.lower() == "o":
                                result = self.customer_model.delete_customer(
                                    customer_obj.id
                                )
                                if result:
                                    display_message(
